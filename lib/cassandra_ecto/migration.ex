defmodule Cassandra.Ecto.Migration do
  ...
  @allowed_column_opts [:primary_key, :partition_key,
  ...
  def execute_ddl(repo, command, opts) do
    cql = execute_ddl(command)
    IO.inspect opts
    IO.inspect cql
    {:ok, _} = Connection.query(repo, cql, [], opts)
  end
  defp execute_ddl({command, %Table{options: [as: :type]} = table, columns}) when command in @creates do
    assemble(["CREATE TYPE", if_not_exists(command), quote_table(table),
      "(#{column_definitions(table, columns)})"])
  end
  defp execute_ddl({command, %Table{} = table, columns}) when command in @creates do
  ...
  defp execute_ddl({command, %Index{} = index}) when command in @creates do
  ...
  defp execute_ddl({_command, %Constraint{} = constraint}), do:
    error! nil, "Cassandra adapter does not support constraints"
  defp execute_ddl({:alter, %Table{} = table, changes}), do:
    assemble([alter(table), column_changes(table, changes)])
  defp execute_ddl({:rename, %Table{} = table, old, new}), do:
    assemble([alter(table), "RENAME", quote_name(old), "TO", quote_name(new)])
  defp execute_ddl({:rename, %Table{} = old, %Table{} = new}), do:
    error! nil, "Cassandra adapter does not support table renaming"
  defp execute_ddl(string) when is_binary(string), do: string
  defp execute_ddl({command, %Table{options: [as: :type]} = table}) when command in @drops, do:
    drop("TYPE", table, command)
  defp execute_ddl({command, %Table{} = table}) when command in @drops, do:
    drop("TABLE", table, command)
  defp execute_ddl({command, %Index{} = index}) when command in @drops, do:
